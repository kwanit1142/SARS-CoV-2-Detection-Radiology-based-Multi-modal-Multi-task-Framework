...
def Task_Specific_Embed(Save_Folder_Directory,x=None,ct=None):
	if x is not None and ct is None:
		x_model = keras.models.load_model(Save_Folder_Directory+'/X_Embed_Task.h5',compile=False)
		task_x = keras.models.load_mode(Save_Folder_Directory+'X_Ray_Task_Model.h5',compile=False)
		...
		ct_model = keras.models.load_model(Save_Folder_Directory+'/CT_Embed_Task.h5',compile=False)
		task_ct = keras.models.load_model(Save_Folder_Directory+'/CT_Scan_Task_Model.h5',compile=False)
...
def Shared_Feature_Embed(Save_Folder_Directory,x=None,ct=None):
	if x is not None and ct is None:
		x_model = keras.models.load_model(Save_Folder_Directory+'/X_Embed_Shared.h5',compile=False)
		...
		ct_model = keras.models.load_model(Save_Folder_Directory+'/CT_Embed_Shared.h5',compile=False)
		shared_model = keras.models.load_model(Save_Folder_Directory+'/Shared_Model.h5',compile=False)
		ct_layers = ct_model.layers[:-1]
		sh_layers = shared_model.layers[:-1]
		for j in ct_layers:
...
def Classification(Save_Folder_Directory,x=None,ct=None):
	if x is None and ct is not None:
		classifier_ct = keras.models.load_model(Save_Folder_Directory+'/Classifier_Head_CT_Model.h5',compile=False)
		...
		classifier_x = keras.models.load_model(Save_Folder_Directory+'/Classifier_Head_X_Model.h5',compile=False)
...
if user_prompt==1:
	...
	root_directory = input("Enter the Root Folder Directory Path for Each and Every saved Components of the user-based Pipeline --> ")
